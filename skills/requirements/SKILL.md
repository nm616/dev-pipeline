...
if [ -f ".claude/agents/requirements.md" ]; then
  ...
  echo "  Run /dev-pipeline:dev-pipeline first to generate agents for this project."
fi
if [ -f "PRD.md" ]; then
...
If the requirements agent was **not found** above: stop, tell the user to run `/dev-pipeline:dev-pipeline` first to generate the project agents, then come back.
